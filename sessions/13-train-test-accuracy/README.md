...
**Focus:** Subsetting a series with `filter()`, `filter_index()` and `slice()` (including the
negative-index idiom and the multi-key case); then errors — absolute vs relative; forecast errors vs
residuals; MAE, RMSE, MAPE, sMAPE, MASE and RMSSE; two worked examples (beer, Google) on both training
and test sets, and the manual recomputation of MAE and RMSE checked against `accuracy()`.
